...
<body style="background-color: #FFFFFF; color: #333333; font-family: Arial, sans-serif; font-size: 16px; margin: 0; padding: 0;">
  ...
  <div style="background-color: #FFFFFF; padding: 20px;">
    ...
    <a href="${process.env.website}/verify/${user._id}/${Verifs[1]}" style="background-color: #FF4136; border-radius: 10px; color: #FFFFFF; display: inline-block; font-size: 16px; padding: 20px; text-decoration: none;">Verify Account</a>
    <p style="margin-top: 20px;">If you have any questions or concerns, please don't hesitate to contact us.</p>
            ...
            if (user.Verified==false) {
                ...
                return sendmail.sent?res.json({Access:true, Error:false,Sent:true}):res.status(500).json({Access:true, Error:sendmail.error})
    
            }
            return res.json({Access:true, Error:'User already verified'})
        }
...
router.get('/:userid/:link',async(req,res)=>{
  ...
  try {
      ...
      //checklogins
      const user=auth?auth.length==24?(await userModel.findOne({_id:auth})):false:false
